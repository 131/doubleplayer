...
var DoubleMedia = function (_EventEmitter) {
  ...
  function DoubleMedia(media, container) {
    var develMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    var blendColor = arguments[3];
    var blendOpacity = arguments[4];
    var blendMode = arguments[5];
    var blendPosition = arguments[6];
    var thickness = arguments[7];
    ...
    _this.thickness = thickness || 0.01;

    if (blend_image) {
      var img = $n('img', { src: blend_image + "#" + Math.random(), crossOrigin: 'IVS' });
      img.addEventListener("load", function () {});
      _this.imageBlendDom = img;
    }

    if (media_type == "image") {
      var _img = $n('img', { src: media_url + "#" + Math.random(), crossOrigin: 'IVS' });

      _img.addEventListener("load", function () {
        console.log("Loadedd metadata");
        _this.media_ready(_img, _img.naturalWidth, _img.naturalHeight);
      });
    }
  ...
  _createClass(DoubleMedia, [{
    ...
    value: function computeFrame() {
      ...
      gl.bindTexture(gl.TEXTURE_2D, this.tex2);
      this.imageBlendDom ? gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, this.imageBlendDom) : gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, 1, 1, 0, gl.RGB, gl.UNSIGNED_BYTE, new Uint8Array([0, 0, 0])); // black texture if no image
...
var DoublePlayer = function (_EventEmitter) {
  ...
  _createClass(DoublePlayer, [{
    ...
    key: 'play',
    value: function play(id, develMode) {
      this.index = id;
    ...
    key: 'setBlendMode',
    value: function setBlendMode(blendColor, blendOpacity, blendMode, blendPosition, thickness) {
